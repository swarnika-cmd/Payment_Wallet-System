...
public class WalletService {
        ...
        @Transactional
        @org.springframework.cache.annotation.CacheEvict(value = "wallets", key = "#mobileNumber")
        ...
        @Transactional
        @org.springframework.cache.annotation.CacheEvict(value = "wallets", key = "#mobileNumber")
        public Transaction withdrawMoney(String mobileNumber, BigDecimal amount, String bankAccount,
                        String description) {
                if (amount.compareTo(BigDecimal.ZERO) <= 0) {
                        throw new IllegalArgumentException("Amount must be positive");
                }
                ...
                if (wallet.getBalance().compareTo(amount) < 0) {
                        throw new com.pocketpay.exception.InsufficientBalanceException(
                                        "Insufficient Balance: Available " + wallet.getBalance() + " < Required "
                                                        + amount);
                }
        ...
        @Transactional
        @org.springframework.cache.annotation.CacheEvict(value = "wallets", allEntries = true)
        public void transferMoney(String senderMobile, String receiverMobile, BigDecimal amount) {
                ...
                System.out
                                .println("SUCCESS: Transferred " + amount + " from " + sender.getName() + " to "
                                                + receiver.getName());
        ...
        @org.springframework.cache.annotation.Cacheable(value = "wallets", key = "#mobileNumber")
        public com.pocketpay.entity.Wallet getWalletBalance(String mobileNumber) {
                User user = userRepository.findByMobileNumber(mobileNumber)
                                .orElseThrow(() -> new RuntimeException("User not found"));
